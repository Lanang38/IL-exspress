...
export const tambahNotifikasi = async (req, res) => {
  const { judul_notifikasi, kategori_id, tanggal, deskripsi, poin } = req.body;
  try {
    await query(
      "INSERT INTO notifikasi (judul_notifikasi, kategori_id, tanggal, deskripsi, poin) VALUES (?, ?, ?, ?, ?)",
      [judul_notifikasi, kategori_id, tanggal, deskripsi, poin]
    );
...
export const editNotifikasi = async (req, res) => {
  ...
  try {
    const result = await query(
      "UPDATE notifikasi SET judul_notifikasi = ?, kategori_id = ?, tanggal = ?, deskripsi = ?, poin = ? WHERE notifikasi_id = ?",
      [judul_notifikasi, kategori_id, tanggal, deskripsi, poin, id]
    );
